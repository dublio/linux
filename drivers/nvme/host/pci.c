...
/*
 * This function is called only if the normal .queue_rq() path submitted at
 * least one command, but fewer than all batched commands to dispatch. That
 * path rings the doorbell only at the end of successfully draining the
 * dispatch list, so unconditionally ring the doorbell now since it wasn't done
 * before.
 */
